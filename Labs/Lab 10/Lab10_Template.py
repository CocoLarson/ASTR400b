...
class M33AnalyticOrbit:
    ...
    def __init__(self, filename):
        ...
        self.Vc=180 #cicrular sppen M31 Halo in km/s hw 5
    ...
    def DynamicalFriction(self, Msat , Vc, r, v):
        '''
        This is a method that returns the deceration from dynamical friction
        felt by M33 owing to its motion thorugh the dark amtter hlao of M31
        ...
        Vc : `np.ndarray`
            Max circular velocity of host dark amtter halo km/s
        r : `np.ndarray`
            Position vector of satellite relative to host kpc
        v : `np.ndarray`
            Velocity vector of satellite relative to host km/s

        RETURNS
        -------
        DF : `np.ndarray`
            Deceleration vector owing to dynamical friction  kpc/Gyr^2
        '''
        #magnitude of position
        r_mag=np.sqrt(r[0]**2+r[1]**2+r[2]**2)
        #magnitude of satlite velocity vector
        v_mag=np.sqrt(v[0]**2+v[1]**2+v[2]**2)

        #for Coulomb log
        #bmax: current seperation between M33 M31
        bmax=r_mag

        #bmin: radius of strong encounter
        # G Msat/ Vc**3
    ...
    def M31Accel(self, r, v):
        ...
        ----------
        r : `np.ndarray`
            Position vector to find the acceleration
        v : `np.ndarray`
            Velocity vector of satellite relative to host km/s
...
ax.plot(M33Orbit['t'], M31_M33_V, 'g', lw=5, linestyle="--",label='M31-M33 with DF')
...
""" Assumed herniquist profile, does not include milky way and its effects on DF, Mass of satellight changes"""
